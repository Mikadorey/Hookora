...
@router.get("/admin/milestone-watch")
def get_milestone_watch(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    enforce_admin_access(current_user)
    ...
    next_milestone = next((m for m in milestones if total_users < m), None)
    previous_milestone = 0 if total_users < 10 else max([m for m in milestones if m <= total_users], default=0)

    progress_percent = 100.0
    users_to_next_milestone = 0

    if next_milestone is not None:
        milestone_range = next_milestone - previous_milestone
        current_progress = total_users - previous_milestone
        progress_percent = round((current_progress / milestone_range) * 100, 2) if milestone_range > 0 else 0.0
        users_to_next_milestone = max(0, next_milestone - total_users)
    ...
    return {
        ...
        "users_to_next_milestone": users_to_next_milestone,
