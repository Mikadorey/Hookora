...
def get_recent_hook_texts(db: Session, user_id: int, topic: str, platform: str | None, limit: int = 25) -> List[str]:
    query = db.query(Hook).filter(Hook.user_id == user_id, Hook.topic == topic)

    if platform:
        query = query.filter(Hook.platform == platform)

    recent = query.order_by(Hook.created_at.desc()).limit(limit).all()
    return [hook.content for hook in recent if hook.content]
...
def create_hook(
    ...
    avoid_hooks = get_recent_hook_texts(db, current_user.id, payload.topic, payload.platform)
...
def regenerate_hook(
    ...
    avoid_hooks = get_recent_hook_texts(
        db,
        current_user.id,
        original_hook.topic,
        original_hook.platform,
        limit=30,
    )
