...
def _normalize_interval(interval: str | None) -> str:
    ...
    return normalized if normalized in VALID_INTERVALS else "monthly"
...
def _get_plan_code(plan: str, interval: str) -> str:
    normalized_plan = _normalize_plan(plan)
    normalized_interval = _normalize_interval(interval)

    mapping = {
        ("creator", "monthly"): settings.paystack_plan_creator_monthly.strip(),
        ("creator", "yearly"): settings.paystack_plan_creator_yearly.strip(),
        ("pro", "monthly"): settings.paystack_plan_pro_monthly.strip(),
        ("pro", "yearly"): settings.paystack_plan_pro_yearly.strip(),
    }

    return mapping.get((normalized_plan, normalized_interval), "")


def _get_plan_amount_naira(plan: str, interval: str) -> int:
    normalized_plan = _normalize_plan(plan)
    normalized_interval = _normalize_interval(interval)
    return PLAN_PRICING_NAIRA.get(normalized_plan, {}).get(normalized_interval, 0)


def _get_plan_amount_kobo(plan: str, interval: str) -> int:
    return _get_plan_amount_naira(plan, interval) * 100


def _get_plan_and_interval_from_plan_code(plan_code: str | None) -> tuple[str, str]:
    normalized = (plan_code or "").strip()

    mapping = {
        settings.paystack_plan_creator_monthly.strip(): ("creator", "monthly"),
        settings.paystack_plan_creator_yearly.strip(): ("creator", "yearly"),
        settings.paystack_plan_pro_monthly.strip(): ("pro", "monthly"),
        settings.paystack_plan_pro_yearly.strip(): ("pro", "yearly"),
    }

    return mapping.get(normalized, ("free", "monthly"))
...
def _sync_user_from_paystack_data(
    ...
    *,
    ...
    fallback_interval: str | None = None,
    ...
    resolved_plan, resolved_interval = _get_plan_and_interval_from_plan_code(plan_code)

    if resolved_plan == "free" and fallback_plan in VALID_PLANS:
        resolved_plan = fallback_plan

    if fallback_interval in VALID_INTERVALS:
        resolved_interval = fallback_interval

    user.plan = resolved_plan if resolved_plan in VALID_PLANS else "free"
    user.billing_interval = resolved_interval if resolved_interval in VALID_INTERVALS else "monthly"
...
def checkout_preview(
    ...
    requested_plan = _normalize_plan(payload.plan)
    interval = _normalize_interval(payload.interval)
    plan_amount_naira = _get_plan_amount_naira(requested_plan, interval)

    if requested_plan not in VALID_PLANS or plan_amount_naira <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def create_checkout_session(
    ...
    if (
        ...
        and (current_user.billing_interval or "monthly") == interval
        and (current_user.billing_status or "").lower() in ACTIVE_BILLING_STATUSES
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"You are already on the {requested_plan} {interval} plan.",
        )

    plan_code = _get_plan_code(requested_plan, interval)
    if not plan_code:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Paystack plan code is not configured for the {requested_plan} {interval} plan.",
        )

    full_amount_naira = _get_plan_amount_naira(requested_plan, interval)
    if full_amount_naira <= 0:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Paystack amount is not configured for the {requested_plan} {interval} plan.",
        )
    ...
    reference = _build_reference(current_user.id, f"{requested_plan}_{interval}")
...
async def paystack_webhook(
    ...
    requested_plan = _normalize_plan(metadata.get("requested_plan"))
    requested_interval = _normalize_interval(metadata.get("requested_interval"))
    ...
    if event_type in {"charge.success", "invoice.create", "invoice.update"}:
        ...
        _sync_user_from_paystack_data(
            ...
            fallback_interval=requested_interval,
        ...
        _sync_user_from_paystack_data(
            ...
            user,
            data,
            fallback_plan=requested_plan or _get_plan_and_interval_from_plan_code(_extract_plan_code(data))[0],
            fallback_interval=requested_interval or _get_plan_and_interval_from_plan_code(_extract_plan_code(data))[1],
            billing_status="active",
        )
