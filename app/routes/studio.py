...
from app.services.ai_service import (
    ...
    generate_content_repurposing_outputs,
    generate_ctas,
...
class ContentRepurposerRequest(BaseModel):
    source_text: str = Field(..., min_length=1, max_length=15000)
...
class ViralRewriterRequest(BaseModel):
    original_text: str = Field(..., min_length=1, max_length=12000)
    topic: Optional[str] = Field(default=None, max_length=240)
    platform: Optional[str] = Field(default="General", max_length=50)
    audience: Optional[str] = Field(default=None, max_length=240)
...
def build_metadata(payload, *, language: str, endpoint: str) -> dict:
    return {
        ...
        "content_type_present": bool((getattr(payload, "content_type", "") or "").strip()),
        "style_present": bool((getattr(payload, "style", "") or "").strip()),
        "avoid_phrases_present": bool((getattr(payload, "avoid_phrases", "") or "").strip()),
...
def build_content_repurposer_metadata(
    payload: ContentRepurposerRequest,
    *,
    language: str,
    endpoint: str,
) -> dict:
    return {
        "source_text_length": len(payload.source_text.strip()),
        ...
        "goal_present": bool((payload.goal or "").strip()),
        "brand_voice_present": bool((payload.brand_voice or "").strip()),
        "cta_present": bool((payload.call_to_action or "").strip()),
        "keywords_present": bool((payload.keywords or "").strip()),
        "avoid_phrases_present": bool((payload.avoid_phrases or "").strip()),
        "extra_context_present": bool((payload.extra_context or "").strip()),
        "language": language,
        "endpoint": endpoint,
...
def build_viral_rewriter_metadata(
    payload: ViralRewriterRequest,
    *,
    language: str,
    endpoint: str,
) -> dict:
...
def studio_generate_content_repurposer_outputs(
    payload: ContentRepurposerRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    started = perf_counter()
    tool_name = "content_repurposer"
    language = normalize_language(payload.language)

    try:
        outputs = generate_content_repurposing_outputs(
            payload.source_text,
            topic=payload.topic,
            platform=payload.platform or "General",
            language=language,
        ...
        log_studio_usage_event(
            ...
            metadata_json=build_content_repurposer_metadata(
                payload,
                language=language,
...
def studio_generate_ad_copy(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            tool_name=tool_name,
            status="success",
            output_count=len(ad_copies),
            generation_ms=generation_ms,
            input_mode="rich_topic_platform_offer",
...
def studio_generate_carousels(
    ...
    try:
        carousels = generate_carousel_slides(
            payload.topic,
            payload.platform or "Instagram",
            language=language,
            audience=payload.audience,
...
@router.post("/viral-rewriter")
def studio_generate_viral_rewriter_outputs(
    payload: ViralRewriterRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            generation_ms=generation_ms,
            input_mode="original_text_rewrite",
            metadata_json=build_viral_rewriter_metadata(
                payload,
                language=language,
                endpoint="/studio/viral-rewriter",
            ),
        )
        return response
    except HTTPException:
...
@router.post("/content-briefs")
def studio_generate_content_briefs_outputs(
    payload: RichStudioRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    started = perf_counter()
    tool_name = "content_brief_generator"
    language = normalize_language(payload.language)

    try:
        briefs = generate_content_briefs(
            payload.topic,
            payload.platform or "General",
            language=language,
            audience=payload.audience,
        ...
        log_studio_usage_event(
            ...
            tool_name=tool_name,
            status="success",
            output_count=len(briefs),
            generation_ms=generation_ms,
            input_mode="rich_topic_strategy",
            metadata_json=build_metadata(payload, language=language, endpoint="/studio/content-briefs"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
