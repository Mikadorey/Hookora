...
def studio_analyze_hook_score_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            metadata_json=build_hook_score_metadata(payload, language=language, endpoint="/studio/hook-score"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_retention_intros_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            generation_ms=generation_ms,
            input_mode="rich_topic_platform",
            metadata_json=build_metadata(payload, language=language, endpoint="/studio/retention-intros"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_comment_to_content_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            metadata_json=build_comment_to_content_metadata(payload, language=language, endpoint="/studio/comment-to-content"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_trend_to_content_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            metadata_json=build_trend_metadata(payload, language=language, endpoint="/studio/trend-to-content"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_nigerian_rewrites_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            generation_ms=generation_ms,
            input_mode="source_text_rewrite",
            metadata_json=build_source_text_metadata(payload, language=language, endpoint="/studio/nigerian-rewrites"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_brand_voice_trainer_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            metadata_json=build_brand_voice_metadata(payload, language=language, endpoint="/studio/brand-voice-trainer"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
...
def studio_generate_content_differentiation_route(
    ...
    try:
        ...
        log_studio_usage_event(
            ...
            generation_ms=generation_ms,
            input_mode="rich_topic_platform",
            metadata_json=build_metadata(payload, language=language, endpoint="/studio/content-differentiation"),
        )
        return response
    except HTTPException:
        generation_ms = int((perf_counter() - started) * 1000)
