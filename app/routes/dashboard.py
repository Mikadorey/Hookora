...
def normalize_tool_type(value: str | None) -> str:
    if not value:
        return "other"
...
def build_usage_messages(usage: dict) -> dict:
    ...
    near_saved_limit = (
        saved_limit is not None and saved_remaining is not None and saved_remaining <= 3
    )
    reached_saved_limit = (
        saved_limit is not None and saved_remaining is not None and saved_remaining <= 0
    )
...
def build_cycle_messages(user: User, usage: dict) -> dict:
    ...
    if (
        usage["saved_hook_limit"] is not None
        and usage["saved_hooks_remaining"] == 0
        and days_until_reset <= 3
    ):
        messages.append(
            "Your saved hook space is full, but you can upgrade now or wait for a reset cycle if your plan changes."
        )
...
def build_studio_usage_analytics(
    ...
    for event in usage_events:
        normalized_tool_name = normalize_tool_type(getattr(event, "tool_name", None))
        meta = get_tool_meta(normalized_tool_name)

        if normalized_tool_name not in tool_summary_map:
            tool_summary_map[normalized_tool_name] = {
        ...
        tool_summary_map[normalized_tool_name]["total_runs"] += 1
        tool_summary_map[normalized_tool_name]["total_outputs"] += event.output_count or 0

        if event.status == "success":
            tool_summary_map[normalized_tool_name]["successful_runs"] += 1
        elif event.status == "failed":
            tool_summary_map[normalized_tool_name]["failed_runs"] += 1

        if event.generation_ms is not None:
            tool_summary_map[normalized_tool_name]["generation_values"].append(event.generation_ms)
    ...
    tools = sorted(
        tools,
        key=lambda item: (-item["total_runs"], item["tool_label"])
    )

    most_used_tool = tools[0]["tool_name"] if tools else None
    most_used_tool_label = tools[0]["tool_label"] if tools else None
    ...
    return {
        ...
        "most_used_tool_label": most_used_tool_label,
        "tools": tools,
...
def get_dashboard_data(
    ...
    for asset in studio_assets:
        normalized_tool_type = normalize_tool_type(asset.tool_type)
        studio_tool_counts[normalized_tool_type] = studio_tool_counts.get(normalized_tool_type, 0) + 1
    ...
    combined_activity = [serialize_hook(hook) for hook in hooks] + [
        serialize_studio_asset(asset) for asset in studio_assets
    ]
    ...
    top_tool_types = sorted(
        [
            {
                "tool_type": key,
                "tool_label": get_tool_meta(key)["label"],
                "tool_badge": get_tool_meta(key)["badge"],
                "count": value
            }
            for key, value in studio_tool_counts.items()
        ],
    ...
    return {
        ...
        "metrics": {
            ...
            "most_used_studio_tool_label": studio_usage_analytics["most_used_tool_label"],
        },
