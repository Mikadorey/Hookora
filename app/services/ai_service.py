...
def _platform_hook_profile(platform: str) -> dict[str, Any]:
    ...
    if platform in {"youtube"}:
        return {
            ...
            "avoid": "cheap clickbait, fake confession framing, over-specific unsupported claims",
        }
    ...
    if platform in {"youtube shorts", "instagram reels", "tiktok"}:
        return {
            ...
            "avoid": "bloated title phrasing and fake guru energy",
        }
...
def _score_hook_text(text: str, platform: str = "General") -> int:
    ...
    # Useful tension / specificity
    if any(word in lowered for word in BELIEVABILITY_WORDS):
        score += 10

    # Platform-aware helpful words
    if platform_lower == "youtube":
    ...
    # Structural tension
    if re.search(r"\b(why|how|what|the reason|warning|confession|mistake|truth)\b", lowered):
        score += 8

    # Penalize obvious generic stock phrasing
    for pattern in GENERIC_HOOK_PATTERNS:
        if re.search(pattern, lowered):
            score -= 8

    # Penalize exaggerated or unsupported-sounding claims
    for pattern in EXAGGERATED_HOOK_PATTERNS:
        if re.search(pattern, lowered):
            score -= 12

    # Penalize cheap clickbait overload
    ...
    # Penalize too many numbers unless phrased naturally
    if len(re.findall(r"\b\d+\b", cleaned)) >= 2:
        score -= 5

    # Reward believable, practical framing
    if re.search(r"\b(review|rule|numbers|question|mistake|habit|shift|pattern|cost)\b", lowered):
        score += 6
    ...
    *,
    ...
    for item in hooks_raw:
        ...
        normalized.append(
            {
                "text": text,
                "score": _score_hook_text(text, normalized_platform),
                "platform": normalized_platform,
                "family": family,
            }
        )
...
- Avoid fake guru phrasing, unsupported personal claims, and spammy curiosity bait.
...
"""
    ...
    try:
        ...
        raise ValueError(f"Primary hook generation returned too few distinct hooks: {len(normalized)}")
        ...
        try:
            ...
            logger.info("Secondary hook path produced %s distinct hooks before final check.", len(normalized))

            if len(normalized) >= 3:
                logger.info("generate_hooks used secondary OpenAI path successfully.")
                return normalized

            raise ValueError(f"Secondary hook generation returned too few distinct hooks: {len(normalized)}")
