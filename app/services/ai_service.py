...
def _score_hook_text(text: str, platform: str = "General", support_first_person: bool = False) -> int:
    ...
    if not support_first_person:
        for pattern in FIRST_PERSON_PATTERNS:
            if re.search(pattern, lowered):
                score -= 12

    for pattern in WEAK_WARNING_PATTERNS:
        if re.search(pattern, lowered):
            score -= 8
...
def _normalize_title_items(items_raw: Any) -> list[dict[str, Any]]:
    normalized = []
    seen: list[str] = []
...
def _normalize_caption_items(items_raw: Any) -> list[dict[str, Any]]:
    normalized = []
    seen: list[str] = []
...
def _normalize_description_items(items_raw: Any, platform: str) -> list[dict[str, Any]]:
    normalized = []
    seen: list[str] = []
...
def _normalize_script_items(items_raw: Any, platform: str) -> list[dict[str, Any]]:
    normalized = []
    seen: list[str] = []
...
def _normalize_thumbnail_result(raw: Any, topic: str, thumbnail_text: str) -> dict[str, Any]:
    ...
    for item in suggestions_raw:
        ...
        suggestion_score = _score_thumbnail_text(text)
        suggestions.append(
            {
                "text": text,
...
- Prioritize clarity, curiosity, and relevance.
- Make them feel specific to the audience and goal.
- Avoid repetitive title shells and avoid generic listicles.
...
"""
    ...
    def normalize_fn(raw):
        return _normalize_title_items(raw.get("titles", []) if isinstance(raw, dict) else [])
...
"""
    ...
    def normalize_fn(raw):
        return _normalize_caption_items(raw.get("captions", []) if isinstance(raw, dict) else [])
...
"""
    ...
    def normalize_fn(raw):
        return _normalize_description_items(raw.get("rewrites", []) if isinstance(raw, dict) else [], normalized_platform)
...
"""
    ...
    def normalize_fn(raw):
        return _normalize_script_items(raw.get("scripts", []) if isinstance(raw, dict) else [], normalized_platform)
...
"""
    ...
    def normalize_fn(raw):
        return _normalize_thumbnail_result(raw, topic, thumbnail_text)
