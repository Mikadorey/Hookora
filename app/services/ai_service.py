...
def _has_unrealistic_claims(text: str) -> bool:
    lowered = text.lower()
    for pattern in REALISM_BAD_PHRASES:
...
def _has_generic_marketing(text: str) -> bool:
    lowered = text.lower()
    for pattern in GENERIC_MARKETING_PHRASES:
        if re.search(pattern, lowered):
            return True
    return False
...
- Avoid repetitive title shells, fake case-study claims, and generic marketing hype.
- {first_person_rule}
...
- Avoid fake testimonials, fake stories, and hype-heavy language.
- {first_person_rule}
...
- Avoid hype-heavy marketing language and unsupported story claims.
- {first_person_rule}
- Each item should contain:
  - title
  - text
...
- Avoid fake guru energy, inflated claims, and unsupported personal-story wins.
- {first_person_rule}
- Each item should contain:
  - title
  - style
