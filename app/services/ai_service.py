...
def _fallback_hooks(topic: str, platform: str, template: str) -> list[dict[str, Any]]:
    ...
    bank = {
        "curiosity": [
            ...
            f"The {topic} mistake that kills results fast",
            f"What actually works for {topic} on {platform}",
            f"Why most people stay stuck with {topic}",
            f"The truth about growing with {topic}",
            f"Before you post about {topic}, read this",
        ],
        "authority": [
            f"This is how high-performers handle {topic}",
            f"The smarter way to approach {topic}",
            f"What experts understand about {topic}",
            f"How to make {topic} work consistently",
            f"The strategic side of {topic} nobody teaches",
            f"Most advice on {topic} is too shallow",
        ],
        "problem": [
            f"Your {topic} is underperforming for one reason",
            f"The biggest mistake people make with {topic}",
            f"Why your {topic} content is getting ignored",
            f"Stop doing this if you want better {topic}",
            f"The hidden bottleneck in your {topic} strategy",
            f"What ruins {topic} faster than people realize",
        ],
        "benefit": [
            f"How better {topic} creates faster growth",
            f"The easiest upgrade you can make to {topic}",
            f"How to turn {topic} into stronger results",
            f"Simple changes that make {topic} perform better",
            f"What happens when you improve {topic} properly",
            f"A better {topic} strategy changes everything",
        ],
        "controversy": [
            f"Most {topic} advice is wrong",
            f"Why I disagree with common {topic} advice",
            f"The unpopular truth about {topic}",
            f"You're overcomplicating {topic}",
            f"Not everyone should approach {topic} this way",
            f"The problem with how people teach {topic}",
        ],
        "story": [
            f"I changed one thing in {topic} and everything shifted",
            f"The moment I realized my {topic} strategy was weak",
            f"What one bad {topic} decision taught me",
            f"I used to misunderstand {topic} completely",
            f"The turning point in how I approached {topic}",
            f"What happened when I simplified {topic}",
        ],
    }
...
def _fallback_titles(topic: str) -> list[dict[str, Any]]:
    ...
    items = [
        ...
        f"{topic}: The Better Way to Start",
    ]
    return [
...
"""
    try:
        ...
        if len(normalized) < 4:
            raise ValueError("Too few distinct hooks returned.")
