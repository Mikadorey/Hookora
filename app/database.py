    ...
    if "users" in tables:
        ...
        if "referral_source_locked_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE users ADD COLUMN referral_source_locked_at DATETIME"
                    )
                )
    ...
    if "referrals" in tables:
        ...
        if "signup_reward_awarded_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE referrals ADD COLUMN signup_reward_awarded_at DATETIME"
                    )
                )
        ...
        if "paid_reward_awarded_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE referrals ADD COLUMN paid_reward_awarded_at DATETIME"
                    )
                )
        ...
        if "created_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE referrals ADD COLUMN created_at DATETIME"
                    )
                )
                connection.execute(
                    text(
    ...
    if "reward_credit_ledger" in tables:
        ...
        if "metadata_json" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE reward_credit_ledger ADD COLUMN metadata_json JSON"
                    )
                )
        ...
        if "created_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE reward_credit_ledger ADD COLUMN created_at DATETIME"
                    )
                )
                connection.execute(
                    text(
    ...
    if "wallet_ledger" in tables:
        ...
        if "metadata_json" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE wallet_ledger ADD COLUMN metadata_json JSON"
                    )
                )
        ...
        if "created_at" not in columns:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "ALTER TABLE wallet_ledger ADD COLUMN created_at DATETIME"
                    )
                )
                connection.execute(
                    text(
